...
public class GraficaRegresion {
    ...
    public ChartPanel generarGrafica(){
        ...
        JFreeChart chart = ChartFactory.createXYLineChart(
            "Regresión Temperatura Barranquilla",
            "Fecha",
            "Temperatura",
            dataset,
    ...
    public XYDataset crearDataset(){
        ...
        for (int i = 0; i < datos.size(); i++) {
            ...
            series1.add(temp.getIdFecha(), temp.getTemperatura());
        }
        XYSeries series2 = new XYSeries("Regresión");
        for (int i = 0; i < datos.size(); i++) {
            temp=(Punto)datos.get(i);
            double pred=coef[0]*temp.getIdFecha()+coef[2];
            series2.add(temp.getIdFecha(), pred);
        }
